...
//						noteService.saveAll(notesTuple);
						noteService.saveRawNotes(notesTuple.getRawNotes());
						noteService.saveTaggedNotes(notesTuple.getTaggedNotes());
						noteService.saveUniqueNotes(notesTuple.getUniqueNotes());
					}
	...
	private <T extends Note> T parseNote(T note, CSVRecord record) {
		try {
			...
			note.setSourceId(Long.valueOf(record.get(Configuration.HEADER_ID)));
